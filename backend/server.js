...
function runtimeHtml(source, fileName) {
  ...
  const enhancements = fileName === 'dashboard.html' ? `${RUNTIME_BRIDGE}\n${BUDGET_SELECTOR_BRIDGE}` : RUNTIME_BRIDGE;
...
function createServer(overrides = {}) {
  ...
  const context = { config, store, auth: createAuthService(store, config), dashboard: createDashboardService(store), aiOrchestrator };
  return http.createServer(async (request, response) => {
    try {
      ...
      response.writeHead(200, { 'Content-Type': type, 'X-Content-Type-Options': 'nosniff', 'Cache-Control': extension === '.html' ? 'no-store' : 'public, max-age=3600' });
